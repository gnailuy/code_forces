...
9. Next Permutation@31: 给定一个数组，可能存在重复字符，求其字典序的下一个组合；算出所有组合太慢了，考虑到字典序，先自右向左找到第一个 a[i-1] < a[i] 的元素，然后用右方第一个大于 a[i-1] 的元素替代 a[i-1]，最后求 a[i] 及其右方所有元素的最小排列即可，也就是右方升序排序，考虑到已经是降序了，所以倒序一下即可；
10. Longest Valid Parentheses@32: 给定一个字符串，包含左右括号，求能够配成对的左右括号最大长度：使用一个栈，保存括号字符或者一个数字，遇到左括号压栈，遇到右括号则向左找第一个左括号，当左右括号配成对，压栈一个数字；向左找左括号时候，可以把连续的数字合并起来；最后遍历栈里的对象，找最大的数字即可；
11. Sudoku Solver@37: 保证数独有唯一解，求这个唯一解：使用 Backtracking，遍历所有未填的点，依次尝试所有 9 个可能数值，每次尝试判断是否 Valid，如果 Valid 则继续递归尝试，如果 Invalid，或者递归尝试失败，那么继续尝试下一个数值；非唯一解时也可以求出一个解，如果想求出其他解，可以每次求出解后先保存解，然后继续循环；
12. Combination Sum@39: 给定一个 Unique 的正数数组，以及一个正数 target，数组里的数可以使用任意多次，求出组合出 target 的所有可能性；对每个数，循环 0~最大可能使用次数，然后递归后面的数组，遇到满足条件的组合就加入结果集合，深度优先遍历解空间；
13. Combination Sum 2@40: 给定一个非 Unique 的正数数组，以及一个正数 target，数组里的数可以使用一次，求出组合出 target 的所有可能性，结果不能重复；构造 prefix list，result list 以及 met set，对于每一个数，考虑包含它和不包含它两种情况，向后递归；
14. Trap Rain Water@42: 给定一个整数数组，表示 Bar 高度，求这些 Bar 能存多少雨水；用一个 Stack 保存 Bar 的 Index，然后从左向右遍历 Bar，遇到比栈顶低的就压栈，遇到比栈顶高的，则一个一个弹出栈，水量加上当前元素和栈顶元素 Index 距离乘以高度差；
15. String Multiply@43: 给定两个字符串表示的整数，求乘积字符串，当然不能用 BigInteger；我实现了一个字符串乘以字符的算法，然后实现了字符串相加的算法，合并到一起就实现了字符串相乘；
16. Jump Game II@45: 给定一个数组，数组中的数是从当前数组可以向前跳的最大步数，初始位于数组开头，求最小的跳到数组结尾的步数，假设结果存在；一个 O(n) 的办法使用贪心策略，每一次循环当前所在位置 start 到当前能到达的 furthest 位置，循环中找到新的 furthest 位置，然后从上次循环的结尾开始到新的 furthest 继续循环，直至到达数组尾部；
