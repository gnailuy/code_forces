...
class TaskScheduler {
    class TaskCount implements Comparable<TaskCount> {
        char task; // Task Name
        int count; // Task Count or Task Last Runtime
    ...
    public int leastInterval(char[] tasks, int n) {
        ...
        // Put all tasks in PQ, ordered by the count
        PriorityQueue<TaskCount> taskQueue = new PriorityQueue<TaskCount>(taskCount.size(), Collections.reverseOrder());
        for (char task: taskCount.keySet()) {
            taskQueue.add(new TaskCount(task, taskCount.get(task)));
        }

        // Cooling down tasks and their last run time
        PriorityQueue<TaskCount> taskTime = new PriorityQueue<TaskCount>();

        // Loop
        while (true) {
            // Get the task of the largest count
            while (!taskQueue.isEmpty()) {
                TaskCount tc = taskQueue.poll();
                intervals += 1;
                // System.out.println(tc.task);
                // Move tasks from cooling queue to task queue if possible
                while (true) {
                    TaskCount tt = taskTime.peek();
                    if (null != tt && intervals - tt.count >= n) {
                        tt = taskTime.poll();
                        taskQueue.add(new TaskCount(tt.task, taskCount.get(tt.task)));
                    } else {
                        break;
                    }
                }
                if (tc.count > 1) {
                    taskCount.put(tc.task, tc.count-1);
                    taskTime.add(new TaskCount(tc.task, intervals));
                }
            }
            // If task queue is empty, check the waiting tasks, advance to the first one
            if (taskTime.size() > 0) {
                TaskCount tt = taskTime.poll();
                taskQueue.add(new TaskCount(tt.task, taskCount.get(tt.task)));
                // System.out.println(tt.count + n - intervals);
                intervals = tt.count + n;
            } else {
                break;
            }
        }
    ...
    public static void main(String[] args) {
        ...
        for (int k = 0; k < tests.length; k++) {
            char[] test = tests[k];
            int n = ns[k];
            System.out.println(test.length + " " + n);
            for (int i = 0; i < test.length; i++) {
                System.out.print(test[i] + " ");
            }
            System.out.println();
            System.out.println(ts.leastInterval(test, n));
            System.out.println();
        }
    }
}
