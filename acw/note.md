...
54. 字符集大小有限时，可以选用最小比特长度编码，达到压缩的效果；
55. 二进制计数编码：当连续一和连续零频率较高时，可以使用零一的个数编码；
56. 哈夫曼编码：首先数字符频率，然后将每个字符看作独立子树，按照频率加入最小优先队列，依次取最小两个结合构建新结点并加回PQ；左零右一编码叶子结点字符；
57. LZW
58. 粒子碰撞事件模拟，用优先队列保存事件，按照时间顺序执行，惰性忽略动态变无效的事件；
59. B-Tree: M degree 的 B-Tree 中每个内部节点保存 M/2 到 M-1 个 Key，分层指向外部节点上的数据本身；
60. SuffixArray 寻找最长重复字串：建立字符串的后缀数组，3WayStringSort 排序，相邻两个字串求最长公共前缀，取最大一个；
